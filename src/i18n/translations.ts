...
export const translations = {
  en: {
    ...
    privacy: {
      ...
      sectionOverviewText2:
        'That has consequences for this policy, so they are stated up front: enough. itself sets no cookies, loads no third-party scripts, fonts, widgets or tracking pixels (its Content-Security-Policy allows connections only to the origin the app is served from and, for the backend, to the wildcard hosts https://*.supabase.co and wss://*.supabase.co — it limits which hosts can be reached at all, but it does not pin the browser to the project host of this instance), shows your conversation partner no read receipts (your read position is stored in your own row alone, and the database policies open that row to your account only: no other user can read it, while the operator, who holds the database, can — section 6), requests no address book, camera, microphone or location permission, stores no media, and has no payment or billing data because the service is free. Where data exists, it is described in the sections below; features that a typical messenger policy lists but that enough. does not run are not described here.',
      ...
      sectionE2eeMetadata:
        'What encryption does not hide: end-to-end encryption protects the content of a message, not the metadata around it. The database keeps one row per message, and the operator can therefore see its ID, the ID of the conversation it belongs to, the user ID of the sender, its creation timestamp, its type marker and the length of the stored ciphertext. The recipient is not a column of its own but follows from the two members of that conversation, and the length is the length of the encrypted envelope, not the length of the text as you typed it. Visible as well: the connection graph with its request states and timestamps, the public key material of every account including which one-time prekey was consumed by whom, and your own read marker, deletion markers and blocks. Realtime subscriptions are metadata too, and they are named per screen: while the overview is open the client listens on one channel for changes to your connections, for new and updated messages and for updated profiles; each open chat adds a channel bound to that conversation ID and a channel whose filter names exactly the two participants of that pair for block changes; the settings screen listens for block changes where you are the blocker. The Realtime layer therefore sees which conversation a client of your account is watching at a given moment and sees the identifiers used inside those filters, while the rows it delivers remain restricted by the same database policies as any other read. The public prekeys are stored on the server because a peer otherwise could not start a session while you are offline.',
      sectionE2eeLimits:
      ...
      sectionE2eeExceptions:
        'Documented exceptions, by design: My Notes (your personal notepad) is a self-chat without a second participant and is stored unencrypted in your own database rows. System events (a name change, an accepted connection request, a deleted account) are not a separate, protected channel: a database trigger writes them into the same messages table as ordinary messages, with an empty ciphertext and with the visible text in the unencrypted metadata column of that row, because both sides have to read them without a key. On an instance where conversations existed before encryption was enabled, those older rows may still contain plaintext and are displayed as legacy messages.',
      ...
      sectionBackendText2:
        'The project is configured by the operator for the AWS region eu-central-1 (Frankfurt am Main, Germany). That region is a setting of this instance and not something the application code fixes, so this sentence describes the configuration of the operator rather than a property of every Supabase project. Supabase documents that the primary Postgres database, the Auth service and Storage objects of a project are hosted in the region selected for that project, and that the region choice alone does not settle a data-residency assessment, because backups, logs, Edge Function execution and sub-processors can involve other locations; its published measures state that daily backups of a project are taken by default and that staff are distributed around the world in a follow-the-sun support model. The contracting party for the service is Supabase Pte. Ltd. (65 Chulia Street #38-02/03, OCBC Centre, Singapore 049513): that company acts as our processor under Article 28 GDPR and is also the data importer named in its Data Processing Agreement. That agreement incorporates the EU Standard Contractual Clauses and completes them so that, for those Clauses alone, Irish law and the courts of Ireland apply; the platform itself is operated within the Supabase group, whose US entity is Supabase, Inc. The agreement lists further sub-processors publicly; Supabase states that the agreement is incorporated into its Terms of Service and therefore applies to a customer organisation without a separate signature, while keeping it current with that provider remains the operator\'s responsibility.',
      sectionBackendLogs:
        'Access, connection and log data: each request to the data API and to Realtime carries your access token and your IP address — the refresh token is sent only to the auth service itself, when a session is renewed — and leaves the requested path, HTTP method, headers, timestamps and error codes at the network layer, for delivery, rate limiting and abuse defence. Supabase does not publish a fixed retention period for these technical logs, so we do not state one; its published measures say only that audit logs of interactions with customer projects and traffic-flow logs are kept, and the published Data Processing Agreement provides that the data Supabase processes for us is deleted once the platform agreement ends and a 30-day export period has passed. Backups belong to the documented defaults of the platform, which describes daily backups of a project, so deleted content can persist in a backup for the provider\'s backup window before it is rotated out.',
      ...
      sectionLocalStorageText2:
        'IndexedDB (database "enough-crypto", stores for state, prekeys, ratchet sessions and sealing keys): your identity key pair and prekey material, the Double Ratchet session states, the sealed message cache (the plaintext of the messages you have read or sent on this device, so that they stay readable) and the sealed Offline Read Mode snapshots (your conversation list and the last 40 messages of an opened chat, with their metadata). All of it is stored as AES-256-GCM sealed records under one sealing key per account, and that key is the non-extractable Web Crypto key the browser keeps in the same database: page script can use it, it cannot read it out, and every record additionally authenticates the account and the record type it belongs to. The private identity key of the messenger protocol is not itself a Web Crypto key: it exists only as a serialized key pair inside one of these sealed records and never as raw key material at a place the page can read. None of this is ever transmitted to the backend.',
      sectionLocalStorageText3:
        'LocalStorage (unencrypted, small values): the appearance mode ("enough-theme"), the language ("enough-lang") and the "Enter sends" preference ("enough-enter-to-send"); your local mirrors of your own deletion markers and read markers ("enough-deletions-<userID>" and "enough-read-<userID>"); and the Supabase Auth session under "sb-<project-ref>-auth-token", which holds the access token, the refresh token and basic profile data. The short-lived PKCE verifier used during sign-up and password reset is a separate entry next to it, under "sb-<project-ref>-auth-token-code-verifier" with a per-flow variant of that name, and it is deleted when the flow finishes. One more entry can still exist on a device that used an older release: before the local message cache was encrypted, the message plaintext was kept unencrypted under "enough-msgplain-<userID>"; the current version moves such an entry into the sealed IndexedDB store and deletes it on the first load, so a device that has not opened the app since then can carry readable message text until you clear the site data. sessionStorage carries one technical flag used when the service worker updates. The service worker caches only the static files of the app shell (HTML, JS, CSS, icons, manifest) — no messages, no tokens, no cross-origin responses.',
      ...
      sectionTransfersText:
        'Your data is not sold, not shared with advertisers, and not disclosed to other users beyond what the architecture itself makes visible. Recipients are: Supabase — the contracting entity of its Data Processing Agreement is Supabase Pte. Ltd. in Singapore, while the platform is operated inside the group whose US entity is Supabase, Inc. (database, authentication, Realtime, the contact-form function); GitHub (delivery of the static files, plus its own sub-processors and infrastructure operators); the email service Resend, whose contracting entity is Plus Five Five, Inc. (delivery of the contact email); the email provider of the operator\'s mailbox; and, where the operator is legally obliged to disclose data, the competent authority — for a private operator this will normally run through the processors\' own legal processes rather than through a disclosure routine at enough.',
      sectionTransfersText2:
        'Transfers are described per recipient instead of as a blanket sentence: GitHub — Pages assets are served from GitHub\'s globally distributed infrastructure, with the company seated in the USA; for its own transfers GitHub publishes reliance on its EU-U.S. Data Privacy Framework self-certification and on Standard Contractual Clauses in its Data Protection Agreement. Supabase — the region the operator selected for this project places the primary database and the Auth service in the EU (AWS eu-central-1, Frankfurt), so the messenger data itself is not stored in a third country. The contract for that service is nonetheless held with Supabase Pte. Ltd. in Singapore, which its Data Processing Agreement also names as the data importer, and the published measures describe daily project backups and staff placed around the world, so support and platform operations can involve access from Singapore, from the USA — where the affiliated entity Supabase, Inc. sits — and from other countries. Singapore has no general EU adequacy decision, which is why this transfer rests on the EU Standard Contractual Clauses (Module Two and, where relevant, Module Three) that the Data Processing Agreement incorporates, and for which that agreement sets Irish law and the courts of Ireland as forum, rather than on adequacy; we have not verified a separate adequacy certification for Supabase and therefore do not claim one. Resend — storage in the USA, transfer under Resend\'s Data Processing Addendum (SCCs) and, additionally, its EU-U.S. Data Privacy Framework participation, as published by Resend. Supabase Edge Functions — the contact-form function runs in the region closest to the requester, which can be a third country; that is inherent to the current architecture and is disclosed here rather than papered over. Any future recipient or tracking feature would require a new version of this policy first.',
      sectionSecurityTitle: '14. Security measures — and what they do not cover',
      sectionSecurityText:
        'Server side: authorization through Row-Level Security policies and database triggers rather than through client checks, no service-role or secret key in the browser, fail-closed rules for authentication and for cryptographic state, normalisation and length limits enforced in the database as well as in the form, input handling at the plaintext boundary only (stored ciphertext is never rewritten), HTML escaping and CRLF filtering in the contact function, and a Content-Security-Policy that admits no origin besides the app itself and the Supabase hosts. Client side: AES-256-GCM sealing of local state under a non-extractable per-account Web Crypto sealing key with additional data bound to each record, an atomic compare-and-swap for ratchet state so that a failed commit sends nothing, and a pinned WebAssembly crypto engine whose artifact hashes are verified by checksum in the build pipeline. Only that sealing key is a protected browser key: the identity key pair of the messenger protocol is kept as a serialized record inside the sealed store and is closed by the sealing key rather than by the browser itself.',
  ...
  de: {
    ...
    privacy: {
      ...
      sectionOverviewText2:
        'Das hat Folgen für diese Erklärung, und sie stehen deshalb vorn: enough. setzt selbst keine Cookies, lädt keine Skripte, Schriften, Widgets oder Tracking-Pixel von Dritten (die Content-Security-Policy lässt Verbindungen nur zur Herkunft der App und, für das Backend, zu den Platzhalter-Domains https://*.supabase.co und wss://*.supabase.co zu — sie begrenzt also, welche Hosts überhaupt erreichbar sind, legt den Browser aber nicht auf den Projekt-Host dieser Instanz fest), zeigt deinem Gegenüber keine Lesebestätigungen (dein Lesestatus steht nur in deiner eigenen Zeile, und die Datenbankrichtlinien öffnen diese Zeile ausschließlich für dein Konto: keine andere Person kann sie lesen, der Betreiber als Inhaber der Datenbank schon — siehe Abschnitt 6), verlangt kein Adressbuch, keine Kamera, kein Mikrofon und keinen Standort, speichert keine Medien und kennt keine Zahlungsdaten, weil der Dienst kostenlos ist. Wo Daten existieren, werden sie in den Abschnitten unten beschrieben; wo eine Verarbeitung, die eine übliche Messenger-Erklärung aufführt, in enough. nicht existiert, beschreiben wir sie bewusst nicht.',
      ...
      sectionE2eeMetadata:
        'Was die Verschlüsselung nicht verbirgt: Ende-zu-Ende-Verschlüsselung schützt den Inhalt einer Nachricht, nicht die Metadaten darum. Die Datenbank führt pro Nachricht eine Zeile, und der Betreiber kann deshalb ihre ID, die ID der Unterhaltung, die Benutzer-ID des Absenders, den Zeitstempel, den Typ-Marker und die Länge des gespeicherten Chiffrats sehen. Der Empfänger ist keine eigene Spalte, sondern folgt aus den beiden Mitgliedern der Verbindung, und die Länge ist die des verschlüsselten Pakets, nicht die deines getippten Textes. Ebenso sichtbar: der Verbindungsgraph mit Anfragestatus und Zeitstempeln, die öffentlichen Schlüsselmaterialien aller Konten einschließlich der Information, welcher One-Time-PreKey von wem verbraucht wurde, sowie deine eigenen Lese- und Löschmarkierer und Blockierungen. Auch die Realtime-Abos sind Metadaten, und sie werden pro Bildschirm benannt: Während die Übersicht offen ist, hört der Client auf einem Kanal nach Änderungen an deinen Verbindungen, an neuen und aktualisierten Nachrichten und an aktualisierten Profilen; jeder geöffnete Chat ergänzt einen Kanal, der an die Verbindungs-ID gebunden ist, und einen Kanal, dessen Filter genau die beiden Personen dieses Paars für Blockierungen nennt; die Einstellungsseite hört auf Blockierungen, bei denen du die blockierende Person bist. Die Realtime-Schicht sieht also, welche Unterhaltung ein Client deines Kontos gerade beobachtet, und sie sieht die Kennungen in diesen Filtern; die ausgelieferten Zeilen bleiben durch dieselben Datenbankrichtlinien beschränkt wie jeder andere Lesezugriff. Die öffentlichen PreKeys liegen auf dem Server, weil dein Gegenüber sonst keine Sitzung aufbauen könnte, wenn du offline bist.',
      sectionE2eeLimits:
      ...
      sectionE2eeExceptions:
        'Dokumentierte Ausnahmen, die so gewollt sind: Meine Notizen (das persönliche Notizbuch) sind ein Selbst-Chat ohne zweite Person und werden unverschlüsselt in deinen eigenen Datenbankzeilen gespeichert. System-Ereignisse (Namensänderung, akzeptierte Anfrage, gelöschtes Konto) sind kein eigener, geschützter Kanal: Ein Trigger schreibt sie als normale Zeilen in dieselbe Nachrichtentabelle wie jede Nachricht — mit leerem Chiffrat und dem sichtbaren Text in der unverschlüsselten Metadaten-Spalte der Zeile, weil beide Seiten sie ohne Schlüssel lesen müssen. Auf einer Instanz, auf der es Unterhaltungen schon vor der Aktivierung der Verschlüsselung gab, können diese älteren Zeilen noch Klartext enthalten; sie werden weiterhin als Klartext aus der Vor-Verschlüsselungszeit angezeigt.',
      ...
      sectionBackendText2:
        'Das Projekt läuft in der von dem Betreiber gewählten AWS-Region eu-central-1 (Frankfurt am Main, Deutschland). Diese Region ist eine Einstellung dieser Instanz und nichts, was der Anwendungscode festlegt; der Satz beschreibt also die Konfiguration des Betreibers und keine Eigenschaft jedes Supabase-Projekts. Supabase dokumentiert, dass die primäre Postgres-Datenbank, der Auth-Dienst und Storage-Objekte eines Projekts in der gewählten Region gehostet werden, und dass die Regionenwahl allein keine Standortbewertung ersetzt, weil Backups, Logs, die Ausführung von Edge Functions und Sub-Prozessoren weitere Standorte betreffen können; die veröffentlichten Maßnahmen sehen standardmäßig tägliche Backups eines Projekts vor und beschreiben weltweit verteilte Mitarbeitenden im Follow-the-sun-Support. Vertragspartner für den Dienst ist Supabase Pte. Ltd. (65 Chulia Street #38-02/03, OCBC Centre, Singapore 049513): Das Unternehmen handelt für uns als Auftragsverarbeiter nach Art. 28 DSGVO und ist in seiner Datenverarbeitungsvereinbarung zugleich als Datenimporteur benannt. Diese Vereinbarung enthält die EU-Standardvertragsklauseln und füllt sie so aus, dass für diese Klauseln irisches Recht und der Gerichtsstand Irland gelten. Betrieben wird die Plattform im Supabase-Konzern, dessen US-Gesellschaft die Supabase, Inc. ist. Die Vereinbarung verweist öffentlich auf eine Sub-Prozessor-Liste; nach Angabe von Supabase ist diese Vereinbarung in die Nutzungsbedingungen eingebunden und gilt damit für die Organisation des Betreibers ohne gesonderte Unterzeichnung, während es in der Verantwortung des Betreibers bleibt, sie aktuell zu halten.',
      sectionBackendLogs:
        'Zugriffs-, Verbindungs- und Logdaten: Jede Anfrage an die Daten-API und an Realtime trägt dein Access-Token sowie deine IP-Adresse — das Refresh-Token geht nur an den Auth-Dienst selbst, wenn eine Sitzung erneuert wird — und hinterlässt in der Netzschicht den angefragten Pfad, die HTTP-Methode, Header, Zeitstempel und Fehlercodes: zur Zustellung, zum Rate-Limiting und zur Missbrauchsabwehr. Für diese technischen Logs veröffentlicht Supabase keine feste Aufbewahrungsfrist, deshalb nennen wir keine; die veröffentlichten Maßnahmen sagen nur, dass Audit-Logs über Interaktionen mit Kundenprojekten und Traffic-Flow-Logs vorgehalten werden, und die veröffentlichte Datenverarbeitungsvereinbarung sieht vor, dass die für uns verarbeiteten Daten gelöscht werden, wenn die Plattformvereinbarung endet und eine 30-tägige Exportfrist verstrichen ist. Backups gehören zu den dokumentierten Standardfunktionen der Plattform, die tägliche Projekt-Backups beschreibt; gelöschte Inhalte können deshalb bis zum Ende des Backup-Zeitraums beim Anbieter nachwirkend vorhanden sein.',
      ...
      sectionLocalStorageText2:
        'IndexedDB (Datenbank „enough-crypto“ mit den Stores für Zustand, PreKeys, Ratchet-Sitzungen und Versiegelungsschlüssel): dein Identitätsschlüsselpaar und PreKey-Material, die Double-Ratchet-Sitzungszustände, der versiegelte Nachrichtencache (der Klartext der auf diesem Gerät gelesenen und geschriebenen Nachrichten, damit sie lesbar bleiben) und die versiegelten Snapshots des Offline-Lesemodus (deine Chat-Übersicht und die letzten 40 Nachrichten eines geöffneten Chats inklusive ihrer Metadaten). All das liegt als mit AES-256-GCM versiegelter Datensatz vor, aufgeschlossen durch je einen Versiegelungsschlüssel pro Konto, und dieser Schlüssel ist der nicht exportierbare Web-Crypto-Schlüssel, den der Browser in derselben Datenbank hält: Ein Skript kann ihn benutzen, aber nicht auslesen, und jeder Datensatz authentisiert zusätzlich das Konto und den Datensatztyp, zu dem er gehört. Der private Identity-Key des Messenger-Protokolls ist selbst kein Web-Crypto-Schlüssel: er existiert ausschließlich als serialisiertes Schlüsselpaar innerhalb eines dieser versiegelten Datensätze und nie als rohes Schlüsselmaterial an einer Stelle, die die Seite lesen kann. Keiner dieser Werte wird an das Backend übertragen.',
      sectionLocalStorageText3:
        'LocalStorage (unverschlüsselt, kleine Werte): Darstellungsmodus („enough-theme“), Sprache („enough-lang“) und die Einstellung „Enter sendet“ („enough-enter-to-send“); deine lokalen Spiegel deiner eigenen Lösch- und Lesemarkierer („enough-deletions-<Benutzer-ID>“ und „enough-read-<Benutzer-ID>“); außerdem die Supabase-Auth-Sitzung unter „sb-<projekt-ref>-auth-token“ mit Access-Token, Refresh-Token und grundlegenden Profildaten. Der kurzlebige PKCE-Verifier für Registrierung und Passwort-Reset liegt als eigener Eintrag daneben, unter „sb-<projekt-ref>-auth-token-code-verifier“ samt einer Variante dieses Namens pro Vorgang, und er wird nach Abschluss gelöscht. Ein weiterer Eintrag kann auf einem Gerät bestehen, das eine ältere Fassung der App genutzt hat: Bevor der lokale Nachrichtencache verschlüsselt war, stand der Klartext unverschlüsselt unter „enough-msgplain-<Benutzer-ID>“; die aktuelle Version übernimmt einen solchen Eintrag in den versiegelten IndexedDB-Speicher und löscht ihn beim ersten Laden. Ein Gerät, das die App seitdem nicht geöffnet hat, kann deshalb lesbaren Nachrichtentext enthalten, bis du die Site-Daten löschst. sessionStorage enthält einen einzigen technischen Marker für Service-Worker-Aktualisierungen. Der Service Worker cached ausschließlich die statischen Dateien der App (HTML, JS, CSS, Icons, Manifest) — keine Nachrichten, keine Token, keine Antworten von anderen Herkunften.',
      sectionLocalStorageText4:
      ...
      sectionTransfersText:
        'Deine Daten werden nicht verkauft, nicht an Werbetreibende gegeben und gegenüber anderen Nutzern nur so weit offengelegt, wie die Architektur es sichtbar macht. Empfänger sind: Supabase — Vertragspartner der Datenverarbeitungsvereinbarung ist Supabase Pte. Ltd. in Singapur, betrieben wird die Plattform im Konzern, dessen US-Gesellschaft die Supabase, Inc. ist (Datenbank, Authentifizierung, Realtime, die Kontaktformular-Funktion); GitHub (Auslieferung der statischen Dateien samt eigener Sub-Prozessoren); der E-Mail-Dienst Resend, vertragliches Unternehmen Plus Five Five, Inc. (Zustellung der Kontakt-Mail); der E-Mail-Anbieter des Betreiber-Postfachs; und soweit der Betreiber gesetzlich zur Offenlegung verpflichtet ist, die zuständige Behörde — bei einem privaten Betreiber läuft das in der Regel über die Verfahren der Anbieter statt über eine Auskunftsroutine bei enough.',
      sectionTransfersText2:
        'Übermittlungen werden pro Empfänger beschrieben statt in einem Pauschalsatz: GitHub — Pages-Auslieferung über die global verteilte Infrastruktur von GitHub mit Sitz in den USA; für eigene Drittlandübermittlungen veröffentlicht GitHub die Selbstzertifizierung nach dem EU-US Data Privacy Framework und Standardvertragsklauseln in seiner Datenvereinbarung. Supabase — die von dem Betreiber gewählte Region legt die primäre Datenbank und den Auth-Dienst dieser Instanz in die EU (AWS eu-central-1, Frankfurt), die Messenger-Daten selbst liegen also nicht in einem Drittland. Der Vertrag über diesen Dienst läuft jedoch mit Supabase Pte. Ltd. in Singapur, das in der Datenverarbeitungsvereinbarung auch als Datenimporteur benannt ist; die veröffentlichten Maßnahmen beschreiben tägliche Projekt-Backups und weltweit verteilte Mitarbeitende, sodass Support- und Plattformbetrieb Zugriffe aus Singapur, aus den USA — dort sitzt die verbundene Gesellschaft Supabase, Inc. — und aus weiteren Ländern betreffen kann. Für Singapur existiert kein allgemeiner Angemessenheitsbeschluss der EU, deshalb stützt sich diese Übermittlung auf die EU-Standardvertragsklauseln (Modul zwei, soweit relevant Modul drei), die die Datenverarbeitungsvereinbarung enthält und für die sie irisches Recht sowie den Gerichtsstand Irland festlegt, und nicht auf Angemessenheit; eine eigenständige Angemessenheits-Zertifizierung von Supabase haben wir nicht geprüft und behaupten sie deshalb nicht. Resend — Speicherung in den USA, Übermittlung auf Basis der Datenverarbeitungsvereinbarung von Resend (SCCs) und zusätzlich dessen DPF-Teilnahme, wie von Resend veröffentlicht. Supabase Edge Functions — die Kontaktformular-Funktion läuft in der Region nahe der anfragenden Person, die ein Drittland sein kann; das folgt aus der gewählten Architektur und wird hier offengelegt statt verschwiegen. Jeder zusätzliche Empfänger oder Tracking-Mechanismus würde vorher eine neue Fassung dieser Erklärung erfordern.',
      sectionSecurityTitle: '14. Sicherheitsmaßnahmen — und was sie nicht abdecken',
      sectionSecurityText:
        'Serverseitig: Autorisierung über Row-Level-Security-Richtlinien und Datenbank-Trigger statt über Prüfungen im Client, kein Service-Role- oder Secret-Key im Browser, fail-closed-Regeln für Authentifizierung und kryptografischen Zustand, Normalisierung und Längenbegrenzung sowohl in der Datenbank als auch im Formular, Eingabehärtung nur an der Klartext-Grenze (gespeicherte Chiffrate werden nie umgeschrieben), HTML-Escaping und CRLF-Filter in der Kontaktformular-Funktion sowie eine Content-Security-Policy, die außer der App selbst nur die Supabase-Hosts zulässt. Clientseitig: AES-256-GCM-Versiegelung des lokalen Zustands unter einem nicht exportierbaren Web-Crypto-Versiegelungsschlüssel pro Konto mit datensatzgebundenen Zusatzdaten, ein atomarer Vergleich-und-Austausch für den Ratchet-Zustand, sodass ein fehlgeschlagener Commit nichts sendet, und eine festgelegte WebAssembly-Krypto-Engine, deren Artefakt-Hashes in der Build-Pipeline geprüft werden. Vor dem Auslesen geschützt ist nur dieser Versiegelungsschlüssel: Das Identitätsschlüsselpaar des Messenger-Protokolls liegt als serialisierter Datensatz im versiegelten Speicher und wird durch den Versiegelungsschlüssel verschlossen, nicht durch den Browser.',
      sectionSecurityText2:
