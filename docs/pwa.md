...
| `src/lib/__tests__/pwa-chrome-color.test.mjs` | Regression guard that the four chrome-colour channels agree, that the two manifest variants differ only in the chrome colours, and that the worker/`index.html`/`theme.ts` legs of the themed manifest hold (`npm run test:pwachrome`) |
...
| `scripts/pwa-plugin.ts` | Vite plugin — emits content-hashed `dist/sw.js` + `sw-build.json` after each production build, with the theme-aware manifest handler |
...
| `src/lib/theme.ts` | Syncs every `theme-color` meta, the manifest link variant, the worker's theme record, and the used `color-scheme` with light/dark |
...
Why a manifest variant works where the metas do not: Chrome on Android
compiles a standalone PWA into a **WebAPK** at install time and paints the
status bar and the gesture navigation bar of that window from the manifest
metadata alone; the `color_scheme_dark` member cannot help here because it
keys off the operating-system preference, which is exactly the mismatch that
was reported (device light, app explicitly dark on Samsung OneUI /
Android 14+).
...
Only relevant if the manifest channel turns out not to work on a device: the
alternative is drawing the app *under* the status bar. Chromium issue
407420295 (and related Gerrit changes) track edge-to-edge rendering with
short-edges cutout mode for installed WebApps/PWAs. In current stable Chrome
on Android, standalone PWAs (`display: standalone`) draw within a viewport
bounded below the status bar unless using `display: fullscreen` (which hides
the clock). `enough.` already pads `env(safe-area-inset-top)` in
`.home-screen`, `.chat-screen`, `.settings-*` and `.legal-header`, and paints
`<html>`, so as soon as Chromium ships edge-to-edge status bar pass-through
for standalone apps, the UI will blend seamlessly without additional layout
work. Not pursued further in this change: the evidence says the manifest is
the working channel, so the app must not pay the layout cost twice.
